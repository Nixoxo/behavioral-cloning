...
def telemetry(sid, data):
    ...
    transformed_image_array = pipeline(image)
    #transformed_image_array = image_array[None, :, :, :]
    #transformed_image_array = imresize(image_array, (32, 64))
    transformed_image_array = transformed_image_array.reshape((1, 32, 64, 3))
    # This model currently assumes that the features of the model are just the images. Feel free to change this.

    prediction = model.predict(transformed_image_array, batch_size=1)
    steering_angle = round(float(prediction),3)
